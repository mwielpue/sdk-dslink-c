...
static void handle_conn(Broker *broker, HttpRequest *req, Socket *sock)
{
    ...
    char buf[1024];
    int len = snprintf(buf, sizeof(buf) - 1, CONN_RESP, (int)strlen(data), data);
    buf[len] = '\0';
    dslink_free(data);
...
static int handle_ws(Broker *broker, HttpRequest *req, Client *client)
{

    size_t len = 0;
    char *dsId = NULL;
    const char *key =
        broker_http_header_get(req->headers, "Sec-WebSocket-Key", &len);
    if (!key)
    {
        goto fail;
    }
    ...
    if (broker_handshake_handle_ws(broker, client, dsId, auth, accept) != 0)
    {
        goto fail;
    }
...
void broker_close_link(RemoteDSLink *link)
{
    ...
    if (link->client)
    {
        if (link->client->poll)
        {
            uv_close((uv_handle_t *)link->client->poll, broker_free_handle);
        }
    ...
    {
        ref =
            dslink_map_get(link->broker->downstream->children, (void *)link->name);
    }

    broker_remote_dslink_free(link);
    // it's possible that free link still rely on node->link to close related
    // streams so link need to be freed before disconnected from node
    if (ref)
    {
...
static void broker_free(Broker *broker)
{
    if (broker->storage)
    {
...
{
    DownstreamNode *link = node;
    listener_add(&link->on_link_connected, extension_on_link_connected,
                 listener->data);
    listener_add(&link->on_link_disconnected, extension_on_link_disconnected,
                 listener->data);
    return 0;
}

static int broker_init(Broker *broker, json_t *defaultPermission)
{
    ...
    json_object_set_new_nocheck(broker->root->meta, "$is",
                                json_string_nocheck("dsa/broker"));
    ...
    if (!(broker->upstream &&
          broker_node_add(broker->root, broker->upstream) == 0))
    {
    ...
    listener_add(&broker->downstream->on_child_added, extension_on_child_added,
                 broker);
    ...
    if (!(node &&
          json_object_set_new_nocheck(node->meta, "$hidden", json_true()) == 0 &&
...
void broker_stop(Broker *broker)
{
    ...
    if (list_is_not_empty(&broker->extensions))
    {
        ...
        dslink_list_foreach(&broker->extensions)
        {
            ...
            if (uv_dlsym(extension->handle, "deinit_ds_extension",
                         (void **)&deinit_function) == 0)
            {
...
int broker_init_extensions(Broker *broker, json_t *config)
{
    ...
    else
    {
        ...
        if (uv_dlsym(extension->handle, "init_ds_extension",
                     (void **)&init_function) != 0)
        {
        ...
        if (extensions_https && json_boolean_value(extensions_https))
        {
            ...
            broker->extensionConfig.brokerUrl =
                setHostFrom("https", httpsHost, httpsPort);
        }
        else
        {
            if (!httpEnabled)
            ...
            broker->extensionConfig.brokerUrl =
                setHostFrom("http", httpHost, httpPort);
        }

        broker->extensionConfig.loop = mainLoop;

        if (init_function(broker->sys, &broker->extensionConfig,
