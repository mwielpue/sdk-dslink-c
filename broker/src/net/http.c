...
static void broker_http_parse_uri(HttpUri *uri, char *data) {
...
const char *broker_http_header_get(const char *headers, const char *name,
                                   size_t *len) {
...
int broker_http_parse_req(HttpRequest *req, char *data) {
  log_debug("Parsing request: %s\n", data);
  char *loc = strstr(data, " ");
  if (!loc) {
    log_debug("There is no loc\n");
    return 1;
  }
  *loc = '\0';
  req->method = data;
  ...
  loc = strstr(data, " ");
  if (!loc) {
    log_debug("There is no second loc\n");
    return 1;
  }
  *loc = '\0';
  log_debug("Parsing URI\n");
