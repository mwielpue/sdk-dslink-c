...
static void mbed_debug(void *ctx, int level, const char *file, int line,
                       const char *str)
{
  ((void)level);
  ((void)ctx);
...
static void broker_server_free_client(uv_poll_t *poll)
{
...
static void broker_server_client_ready(uv_poll_t *poll, int status,
                                       int events)
{
  (void)status;
  Client *client = poll->data;
    ...
    if (client && (events & UV_WRITABLE))
    {
      ...
      if (link && link->ws)
      {
        ...
        else
        {
          ...
          uv_poll_start(poll, UV_READABLE | UV_WRITABLE,
                        broker_server_client_ready);
          int stat = wslay_event_send(link->ws);
          if (stat != 0 || link->ws->read_enabled == 0 ||
  ...
  if (client && (events & UV_WRITABLE))
  {
    ...
    if (link && link->ws)
    {
      ...
      else
      {
        log_debug("Enabling READ/WRITE poll on client\n");
        uv_poll_start(poll, UV_READABLE | UV_WRITABLE,
                      broker_ssl_server_client_ready);
        int stat = wslay_event_send(link->ws);
        if (stat != 0 || link->ws->read_enabled == 0 ||
            link->ws->write_enabled == 0)
        {
...
static void broker_server_new_client(uv_poll_t *poll, int status, int events)
{
  (void)status;
  (void)events;
  ...
  if (mbedtls_net_accept(&server->srv, &client->sock->socket_ctx, NULL, 0,
                         NULL) != 0)
  {
  ...
  if (uv_poll_init(loop, clientPoll, client->sock->socket_ctx.fd) != 0)
  {
...
static void broker_ssl_server_new_client(uv_poll_t *poll, int status,
                                         int events)
{
  (void)status;
  (void)events;
  ...
  if ((ret = mbedtls_ctr_drbg_seed(
           &sslSocket->drbg, mbedtls_entropy_func, &sslSocket->entropy,
  ...
  if ((ret = mbedtls_ssl_config_defaults(
           &sslSocket->conf, MBEDTLS_SSL_IS_SERVER,
  ...
  mbedtls_ssl_conf_cert_profile(&sslSocket->conf,
                                &mbedtls_x509_crt_profile_next);

  static int preset_suiteb_hashes[] = {MBEDTLS_MD_SHA512, MBEDTLS_MD_SHA384,
                                       MBEDTLS_MD_SHA256, MBEDTLS_MD_NONE};
  mbedtls_ssl_conf_sig_hashes(&sslSocket->conf, preset_suiteb_hashes);

  static int ciphersuites[] = {MBEDTLS_TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
  ...
  mbedtls_ssl_conf_min_version(&sslSocket->conf, MBEDTLS_SSL_MAJOR_VERSION_3,
                               MBEDTLS_SSL_MINOR_VERSION_3);

  mbedtls_ssl_conf_rng(&sslSocket->conf, mbedtls_ctr_drbg_random,
                       &sslSocket->drbg);
  mbedtls_ssl_conf_dbg(&sslSocket->conf, mbed_debug, stdout);

  mbedtls_ssl_conf_ca_chain(&sslSocket->conf, server->srvcert.next, NULL);
  if ((ret = mbedtls_ssl_conf_own_cert(&sslSocket->conf, &server->srvcert,
                                       &server->pkey)) != 0)
  {
  ...
  if (mbedtls_net_accept(&server->srv, &sslSocket->socket_ctx, NULL, 0, NULL) !=
      0)
  {
    log_warn("Failed to accept a client connection\n");
    goto fail_poll_setup;
  ...
  mbedtls_ssl_set_bio(&sslSocket->ssl, &sslSocket->socket_ctx, mbedtls_net_send,
  ...
  log_debug("Generating clientpoll\n");
  uv_poll_t *clientPoll = dslink_malloc(sizeof(uv_poll_t));
  if (!clientPoll)
  {
    goto fail_poll_setup;
...
static int start_http_server(Server *server, const char *host, const char *port,
...
static int start_https_server(SslServer *server, const char *host,
                              const char *port, const char *certFile,
                              const char *certKeyFile, uv_loop_t *loop,
                              uv_poll_t *poll)
{
...
static void stop_server_handler(uv_signal_t *handle, int signum)
{
  const char *sig;
  if (signum == SIGINT)
  {
...
int broker_start_server(json_t *config)
{
  ...
  if (httpEnabled && httpHost && httpPort[0] != '\0')
  {
    ...
    httpActive =
        start_http_server(&httpServer, httpHost, httpPort, mainLoop, &httpPoll);
  }
  ...
  memset(httpsPort, 0, sizeof(httpsPort));
  {
    ...
    if (https)
    {
      ...
      if (enabled && json_boolean_value(enabled))
      {
        ...
        strcat(httpsCertFile,
               json_string_value(json_object_get(https, "certName")));
        strcat(httpsCertKeyFile,
               json_string_value(json_object_get(https, "certKeyName")));
      }
    }
  }
  ...
  if (httpsEnabled && httpsHost && httpsPort[0] != '\0')
  {
    ...
    httpsActive =
        start_https_server(&httpsServer, httpsHost, httpsPort, httpsCertFile,
                           httpsCertKeyFile, mainLoop, &httpsPoll);
  }
  else
    httpsEnabled = 0;
  ...
  //    upstream_connect_conn(&loop, "http://10.0.1.158:8080/conn",
  //    "dartbroker", "cbroker");
